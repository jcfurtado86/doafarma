<?php

declare(strict_types = 1);

namespace App\Policies;

use App\Models\MedicationOffering;
use App\Models\User;

class MedicationOfferingPolicy
{
    /**
     * Determine whether the user can view any models.
     */
    public function viewAny(User $user): bool
    {
        return $user->doctor?->id !== null;
    }

    /**
     * Determine whether the user can view the model.
     */
    public function view(User $user, MedicationOffering $medicationOffering): bool
    {
        return $user->doctor?->id === $medicationOffering->doctor_id;
    }

    /**
     * Determine whether the user can create models.
     */
    public function create(User $user): bool
    {
        return $user->doctor?->id !== null;
    }

    /**
     * Determine whether the user can update the model.
     */
    public function update(User $user, MedicationOffering $medicationOffering): bool
    {
        return $user->doctor?->id === $medicationOffering->doctor_id;
    }

    /**
     * Determine whether the user can delete the model.
     */
    public function delete(User $user, MedicationOffering $medicationOffering): bool
    {
        return $user->doctor?->id === $medicationOffering->doctor_id;
    }

    /**
     * Determine whether the user can restore the model.
     */
    public function restore(User $user, MedicationOffering $medicationOffering): bool
    {
        return false;
    }

    /**
     * Determine whether the user can permanently delete the model.
     */
    public function forceDelete(User $user, MedicationOffering $medicationOffering): bool
    {
        return false;
    }
}
